...
const BulkImportModal: React.FC<BulkImportModalProps> = ({
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < csvData.length; i += chunkSize) {
      ...
      const { data: insertedLeads, error: insertError } = await (supabase as any)
        .from('leads')
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {fields.map(field => (
                      <div key={field.key} className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                        <label htmlFor={`mapping-${field.key}`} className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">

...
const NotificationDropdown: React.FC<NotificationDropdownProps> = ({ userId }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <>
          {/* Backdrop móvil para cerrar al hacer clic fuera (opcional, pero ayuda UX) */}
          <div className="fixed inset-0 z-40 sm:hidden" onClick={() => setIsOpen(false)}></div>

          <div className="fixed left-4 right-4 top-16 sm:absolute sm:right-0 sm:left-auto sm:top-auto mt-2 sm:mt-3 w-auto sm:w-96 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-700 overflow-hidden z-50 animate-scale-in origin-top-right transition-colors duration-200">

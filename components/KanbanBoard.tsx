...
const KanbanBoard: React.FC<KanbanBoardProps> = ({
    ...
    const handleToggleAll = () => {
        if (collapsedColumns.size === statuses.length) {
            setCollapsedColumns(new Set()); // Expandir todo
        } else {
            setCollapsedColumns(new Set(statuses.map(s => s.id))); // Colapsar todo
        }
    };

    const isAllCollapsed = collapsedColumns.size === statuses.length;
            ...
            <div className="flex flex-col h-full">
                ...
                <div className="flex justify-end px-2 mb-2">
                    <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-brand-secondary dark:text-gray-400 dark:hover:text-white transition-colors px-3 py-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700/50"
                        title={isAllCollapsed ? "Expandir todos los tableros" : "Colapsar todos los tableros"}
                    >
                        {isAllCollapsed ? (
                            <>
                ...
                <div className="flex overflow-x-auto pb-6 h-[calc(100dvh-280px)] space-x-4 p-2 snap-x snap-mandatory sm:space-x-6 scroll-smooth items-start custom-scrollbar">
                    {statuses.map((status) => {
                        ...
                        if (isCollapsed) {
                            return (
                                <div
                                    key={status.id}
                                    className="flex-shrink-0 w-16 h-full flex flex-col items-center bg-gray-200 dark:bg-slate-800/80 rounded-2xl border border-gray-300 dark:border-slate-700 py-4 cursor-pointer hover:bg-gray-300 dark:hover:bg-slate-700/80 transition-colors snap-center"
                                ...
                                >
                                    ...
                                    <div className="flex-1 w-full flex items-center justify-center overflow-hidden py-2">
                                        <h3
                                            className="[writing-mode:vertical-lr] rotate-180 whitespace-nowrap font-bold text-gray-500 dark:text-gray-300 text-sm tracking-wide uppercase truncate max-h-full"
                                            title={status.name}
                                        >
                                            {status.name}
                                        </h3>

...
interface LeadToolbarProps {
    ...
    onToggleUnreadMessages?: () => void; // [NEW]
}
                ...
                <div className="flex items-center w-full sm:w-auto gap-3 justify-between sm:justify-end">
                    {viewMode !== 'calendar' && (
                        <div className="flex items-center gap-2">
                            {onToggleUnreadMessages && (
                                <button
                                    onClick={onToggleUnreadMessages}
                                    title="Mostrar solo mensajes sin leer"
                                    className={`relative inline-flex items-center px-4 py-2.5 text-sm font-medium rounded-lg transition-all ${hasUnreadMessages
                                        ? 'bg-green-100 text-green-700 ring-1 ring-green-500/50 dark:bg-green-900/40 dark:text-green-400'
                                ...
                                >
                                    ...
                                    Sin Leer
                                    {hasUnreadMessages && <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-green-500 rounded-full animate-ping"></span>}
                                </button>
                            )}
                            <button

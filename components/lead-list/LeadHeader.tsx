...
interface LeadHeaderProps {
    ...
    globalTotal?: number; // Total real en DB
    ...
    return (
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
            <div>
                ...
                <div className="mt-1 text-sm text-gray-500 dark:text-gray-400 flex items-center gap-2 flex-wrap">
                    {globalTotal > 0 && (
                        <span>Total Global: <span className="font-bold text-gray-900 dark:text-white mr-2">{globalTotal}</span></span>
                    )}
                    <span className={globalTotal > 0 ? "border-l pl-2 border-gray-300 dark:border-gray-600" : ""}>
                        Mostrando: <span className="font-bold text-brand-primary dark:text-blue-400">{loadedLeadsCount}</span> de <span className="font-bold">{totalLeads}</span>
                    </span>
                </div>
            </div>
